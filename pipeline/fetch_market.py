...
def fetch_intraday(tickers: list):
    import yfinance as yf

    log(f"분봉 내려받는 중… {len(tickers)}개 종목 (30분봉, 최근 60일)")
    out = {"interval": "30m", "series": {}}
    for t in tickers:
        try:
            df = yf.download(t, period="60d", interval="30m", auto_adjust=True,
                             progress=False)
            ...
            out["series"][t] = {
                "times": [str(x)[:16].replace("T", " ") for x in df.index],
                "close": [r2(v) for v in df["Close"].to_numpy()],
                "volume": [kvol(v) for v in df["Volume"].to_numpy()],
            }
        except Exception as e:                       # 한 종목 실패가 전체를 막지 않게
            log(f"  {t} 분봉 실패: {e}")
    log(f"분봉 확보: {len(out['series'])}개 종목")
    return out
...
def main(argv=None) -> int:
    ...
    if not args.skip_intraday:
        if args.synthetic:
            write_json("intraday.json", {"interval": "30m", "synthetic": True, "series": {}})
        else:
            write_json("intraday.json", fetch_intraday(INTRADAY_TICKERS))
