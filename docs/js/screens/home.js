 ...
 *
 *  화면이 일곱 개나 되면 어디부터 눌러야 할지 모릅니다. 그래서 커리큘럼의
...
(function (root) {
  ...
  function nextStepPanel(host) {
    ...
    if (!step) {
      ...
      p.body.innerHTML =
        '<p class="small">12단계를 모두 마쳤습니다. 이제 이 사이트에서 배울 수 있는 것은 거의 다 다뤘습니다.</p>' +
  ...
  function curriculumPanel(host) {
    ...
    if (!S.showAll) {
      p.body.appendChild(U.el('div', 'tiny mt', '"전체 보기"를 누르면 12단계 전부와 확인 질문을 볼 수 있습니다.'));
    }
    return p;
  }
