...
(function (root) {
  ...
  const TASKS = [
    ...
    { id: 't3b', title: '내 알파를 쓴다',
      body: '알파 만들기 탭에서 <b>가설을 말로 먼저 정하고</b>(예: "오르되 덜 흔들린 종목이 낫다") ' +
        '식으로 옮겨 시뮬레이션해 보세요. 채점표의 Sharpe·Fitness·회전율 셋을 함께 봐야 합니다. ' +
        'Sharpe만 좋고 회전율이 70%를 넘으면 <b>제출 자체가 되지 않습니다.</b>' },
    { id: 't3c', title: '아이디어를 고르고 고쳐 본다',
      body: '배우기 → <b>알파 아이디어</b>에서 카테고리를 <b>먼저</b> 하나 고르세요(예: "나는 거래량 쪽을 보겠다"). ' +
        '그 안에서 2~3개만 골라 각 항목의 <b>조심할 점</b>에 적힌 실험을 해 보세요. ' +
        '26개를 다 돌려 제일 좋은 것을 고르면 그게 바로 다중검정입니다. ' +
        '고른 이유를 연구 노트에 <b>먼저</b> 적어 두면 나중에 자기를 속이기 어려워집니다.' },
    { id: 't3d', title: '병목이 어디인지 읽는다',
      body: '기준 미달이 나오면 채점표 아래 <b>다음 수</b> 패널이 무엇이 병목인지 짚어 줍니다. ' +
        '제안 하나를 골라 적용하고 다시 돌린 뒤, <b>어느 숫자가 올라가고 어느 숫자가 내려갔는지</b> 적어 보세요. ' +
        '예: 회전율을 낮추면 Fitness는 오르지만 Sharpe가 같이 떨어지는 일이 잦습니다. ' +
        '제안을 순서대로 다 눌러 보는 것은 금지입니다 — 그렇게 얻은 통과는 통과가 아닙니다.' },
    { id: 't3e', title: '기준선 없이는 아무것도 아니다',
      body: '낙폭 반등 탭에서 기본 설정 그대로 <b>분석 실행</b>을 눌러 보세요. ' +
        '"낙폭 후 20일 반등 확률 58%"라는 숫자가 나옵니다. 대단해 보이나요? ' +
        '바로 옆 <b>기준선</b>을 보세요 — 아무 날이나 샀어도 55%입니다. ' +
        '낙폭이 실제로 보탠 것은 그 <b>차이</b>뿐입니다. ' +
        '그리고 1일·3일 보유는 비용을 빼면 마이너스입니다. 왜 그런지 설명해 보세요.' },
    { id: 't3g', title: '열 가지를 한 화면에 놓고 본다',
      body: '낙폭 반등 탭에서 <b>열 가지 전부 비교</b>를 누르고 분석해 보세요. ' +
        '낙폭·RSI·이동평균·급락 등 열 가지 신호의 성적이 한 화면에 나옵니다. ' +
        '<b>1등을 고르지 마세요.</b> 대신 아래 버튼을 눌러 <b>채점 구간에서 순위가 어떻게 바뀌는지</b> 보세요. ' +
        '개발 구간 1등이 채점 구간에서 몇 위인지 적어 보고, 왜 그런지 설명해 보세요. ' +
        '덧붙여 — 200일선 하향 돌파는 개발 구간에서도 <b>기준선보다 나빴습니다</b>. ' +
        '교과서에 나오는 지표라고 다 통하는 것이 아닙니다.' },
    { id: 't3h', title: '시장 탓인지 종목 탓인지 나눠 본다',
      body: '낙폭 반등의 <b>하나 자세히 보기</b>에서 분석하면 "시장 탓인가, 그 종목 탓인가" 패널이 나옵니다. ' +
        '시장(QQQ)도 같이 빠져 있던 날의 반등과, 그 종목만 빠졌던 날의 반등을 갈라서 보여 줍니다. ' +
        '<b>시장이 통째로 빠질 때의 반등은 그 종목의 힘이 아니라 시장의 회복입니다.</b> ' +
        '두 집단의 차이를 적고, 이 신호의 수익이 어느 쪽에서 나온 것인지 판단해 보세요.' },
    { id: 't3i', title: 'AI가 정말 맞히는지 확인한다',
      body: '낙폭 반등 → <b>AI로 예측해 보기</b>에서 로지스틱 회귀로 한 번, 랜덤포레스트로 한 번 돌려 보세요. ' +
        '<b>학습 AUC와 시험 AUC를 둘 다 적으세요.</b> 랜덤포레스트는 학습 AUC가 훨씬 높게 나오는데 ' +
        '시험 AUC는 나아지지 않습니다. 왜 그런지 설명해 보세요.<br>' +
        '그리고 시험 AUC가 0.5보다 낮게 나왔다고 해서 <b>"반대로 쓰면 되겠다"고 생각하면 안 됩니다.</b> ' +
        '왜 안 되는지도 함께 적어 보세요. 알파 만들기의 "설명 없이 부호를 뒤집지 말라"와 같은 이야기입니다.' },
    { id: 't3f', title: '"최적 보유 기간"을 의심한다',
      body: '낙폭 반등에서 개발 구간의 최적 보유 기간을 확인한 뒤, ' +
        '<b>채점 구간에서 돌리기</b>를 눌러 보세요. 최적 기간이 그대로인가요? ' +
        '보유 기간 7가지 중 가장 좋은 것을 고른 것이므로 그 자체가 이미 한 번의 선택입니다. ' +
        '<b>과거의 최적은 미래의 최적이 아닙니다.</b> ' +
        '덧붙여 — 이 화면의 반등 확률은 생존 편향 때문에 실제보다 반드시 높습니다. 왜 그런지 말로 설명해 보세요.' },
  ...
  const GLOSSARY = [
    ...
    ['이벤트 스터디 (Event study)', '어떤 사건(실적 발표, 금리 결정, 급락 등)이 일어난 뒤 며칠 동안 가격이 어떻게 움직이는지 세는 방법. 알파가 <b>매일 전 종목의 순위를 매기는</b> 방식이라면, 이벤트 스터디는 <b>특정 순간 이후를 따라가는</b> 방식입니다. 낙폭 반등 탭이 이 방식입니다.'],
    ['기준선 (Baseline)', '비교 대상. "낙폭 후 반등 확률 58%"는 그 자체로 아무 뜻이 없습니다. 아무 날이나 샀어도 55%였다면 낙폭이 보탠 것은 3%p뿐입니다. <b>기준선 없는 확률은 숫자 장난입니다.</b> 어떤 결과를 보든 "그냥 했으면 얼마였나"를 먼저 물어야 합니다.'],
    ['보유 기간 (Holding period)', '사서 팔 때까지의 기간. 짧으면 거래비용에 먹히고, 길면 신호가 희석됩니다. "최적 보유 기간"을 찾았다면 조심하세요 — 여러 기간 중 제일 좋은 것을 고른 것이므로 그 자체가 이미 한 번의 선택이고, 다음 구간에서는 다른 기간이 최적일 수 있습니다.'],
    ['쿨다운 (Cooldown)', '같은 신호를 다시 세지 않는 유예 기간. 한 번 크게 빠지면 그 뒤 수십 일 동안 매일 "낙폭 -20%"가 성립합니다. 그대로 두면 <b>하나의 하락이 표본 50개로 부풀어</b> 통계가 거짓말을 합니다. 이벤트 스터디에서 반드시 챙겨야 하는 장치입니다.'],
  ...
  const IQC_STAGES = [
    ...
    { n: 3, title: '통과 기준 감 잡기', when: '2~4주',
      ...
      here: '채점표의 눈금이 BRAIN의 합격선과 같은 값입니다. ' +
        ...
        'Fitness는 성과를 올리는 것보다 회전율을 낮추는 편이 빠를 때가 많습니다.<br>' +
        '기준을 넘겼다면 <b>채점 구간에서 확인</b>을 눌러 보세요. 개발 구간에서 좋았던 알파가 ' +
        '처음 보는 12개월에서 무너지는 것을 직접 보게 됩니다. BRAIN에서도 제출 후에야 열리는 정보입니다. ' +
        '단, 여러 번 볼수록 그 구간도 개발 구간이 되므로 확인 횟수가 기록됩니다.' },
