...
(function (root) {
  ...
  function sectorPanel() {
    ...
    note.innerHTML =
      '업종을 여러 개로 나누면, 그중 어딘가는 대개 좋아 보입니다. 나누는 개수가 많을수록 ' +
      '그중 하나가 우연히 좋을 가능성도 함께 커지기 때문이에요. ' +
      '그래서 여기서 제일 좋은 업종을 골라 "이 업종의 낙폭은 잘 반등한다"고 결론 내리기 쉬운데, ' +
      '그 전에 표본 수(괄호 안)를 먼저 보면 좋겠습니다. 30건 미만이면 판단 근거로 삼기 어렵습니다.';
  ...
  function comparePanel(host) {
    ...
    warn.innerHTML =
      '<b>이 표는 1등을 고르는 표가 아닙니다.</b><br><br>' +
      '열 가지를 하나씩 따로 돌려 보고 제일 좋은 것을 골랐다면, 그것이 바로 <b>다중검정</b>이에요. ' +
      '아무 신호가 없는 데이터에서도 열 개 중 하나쯤은 좋아 보이기 마련입니다. ' +
      '열 가지를 한 화면에 나란히 놓은 이유가 여기에 있습니다. 따로 볼 때는 잘 안 보이던 것이 ' +
      '같이 놓으면 눈에 들어옵니다.<br><br>' +
      '순위보다 먼저 볼 것은 <b>표본 수</b>와 <b>비용 차감 열</b>입니다. ' +
      '표본이 30건 미만인 줄은 순위가 몇 등이든 근거로 삼기 어렵고, ' +
      '비용을 빼면 대부분이 0 근처로 내려앉는다는 점도 함께 확인해 보세요.';
  ...
  function aiPanel() {
    ...
    if (root.ML.isRule(A.model)) {
      ...
      rn.innerHTML =
        '지금 고른 것은 AI가 아니라 <b>기준선</b>입니다. ' +
        (A.model === 'alwaysup'
          ? '아무것도 배우지 않고 무조건 오른다고만 답하는 방식이에요. '
          : '동전을 던져서 아무렇게나 답하는 방식이에요. ') +
        'AUC가 0.5 근처로 나오는 것이 정상입니다. ' +
        'AI 모델의 점수가 이 값보다 뚜렷하게 높아야 무언가를 배웠다고 말할 수 있습니다. ' +
        '모델을 비교할 때 이 기준선을 먼저 재 두면 좋겠습니다.';
    ...
    vt.innerHTML = a >= 0.55
      ? '시험 구간 AUC가 <b>' + a.toFixed(3) + '</b>입니다. 금융 데이터에서 0.55는 낮은 값이 아니에요. ' +
        '다만 학습 AUC(' + A.aucTrain.toFixed(3) + ')와 얼마나 차이가 나는지도 같이 보면 좋습니다. ' +
        '차이가 크다면 배웠다기보다 외운 쪽에 가깝습니다.'
      : (a >= 0.52
        ? '시험 구간 AUC가 <b>' + a.toFixed(3) + '</b>입니다. 0.5보다 조금 높긴 하지만, ' +
          '거래비용을 감당하기에는 아직 부족한 수준이에요. ' +
          '학습 AUC는 ' + A.aucTrain.toFixed(3) + '였으니, 배운 것의 상당 부분이 시험 구간에서는 남지 않았습니다.'
        : '시험 구간 AUC가 <b>' + a.toFixed(3) + '</b>입니다. ' +
          (a < 0.5
            ? '0.5보다 낮으니, 모델이 좋게 본 쪽이 실제로는 오히려 나빴다는 뜻입니다. ' +
              '여기서 "그럼 반대로 쓰면 되겠네"라는 생각이 자연스럽게 드는데, 조금 더 살펴보면 그렇지 않아요. ' +
              '방향이 거꾸로인 신호가 있는 것이 아니라, 애초에 신호가 없어서 아무 쪽으로나 빗나간 것이라서 ' +
              '다음 구간에서는 또 아무 쪽으로나 빗나갑니다. ' +
              '무작위 기준선을 골라 봐도 비슷한 모양이 나오는 것으로 확인할 수 있습니다. '
            : '동전 던지기와 큰 차이가 없는 값이에요. ') +
          '학습 구간에서는 ' + A.aucTrain.toFixed(3) + '였는데도 그렇습니다' +
          (A.aucTrain - a > 0.15 ? ' — 배운 것이 시험 구간에서 거의 남지 않았습니다(과적합)' : '') + '. ' +
          '모델이 나쁜 것이 아니라, 이 재료만으로는 개별 반등을 맞히기 어렵다는 뜻입니다. ' +
          '이것도 충분히 의미 있는 결과이니, 그대로 보고하면 됩니다.');
    v.appendChild(vt);
  ...
  function honestyPanel() {
    ...
    add('생존 편향 — 이 화면에서 가장 큰 문제',
      ...
      '즉 우리는 "떨어졌지만 결국 살아남은 종목"만 보고 반등 확률을 재고 있는 셈입니다. ' +
      '그래서 여기 나온 반등 확률은 실제보다 높게 나올 수밖에 없습니다. ' +
      '얼마나 높은지는 이 데이터만으로는 알기 어렵습니다.', 'warn');

    add('표본이 서로 독립이 아닙니다',
      '시장 전체가 빠지는 날에는 수십 종목이 동시에 신호를 냅니다. ' +
      '그 표본들은 사실상 <b>하나의 사건</b>에 가까운데, 통계는 여러 개로 셉니다. ' +
      '그래서 표의 t값은 실제보다 크게 나오는 편이에요. |t|가 2를 넘었다는 것만으로 ' +
      '유의하다고 보기는 어렵습니다. ' +
      '위의 <b>시장 탓인가 종목 탓인가</b> 패널이 이 문제의 한 단면을 보여 줍니다.', 'warn');

    add('여러 번 바꿔 보면 좋은 조합은 대개 나옵니다',
      ...
      '연구 노트에 시도 횟수가 남으니, 발표할 때 몇 번째 조합인지 함께 밝혀 주면 좋겠습니다. ' +
      '그렇게 밝히는 것만으로도 연구의 신뢰도가 올라갑니다.');
  ...
  function introPanel() {
    ...
    d.innerHTML =
      ...
      '<p><b>여기서 가장 중요한 것 하나.</b> 반등 확률 62%라는 숫자만으로는 판단하기 어렵습니다. ' +
      '아무 날이나 샀어도 58%였다면 낙폭이 보탠 몫은 4%p뿐이고, 그 정도는 거래비용에 묻히거든요. ' +
      '그래서 이 화면은 모든 숫자 옆에 <b>기준선</b>을 나란히 놓아 둡니다.</p>' +
      ...
      '하나씩 돌려 보고 제일 좋은 것을 고르기 쉬운데, 그러면 판단이 흐려지기 때문에 ' +
      '처음부터 다 같이 보이도록 만들었습니다.</p>' +
      '<p>한 가지 미리 알아 둘 점은 <b>생존 편향</b>입니다. ' +
      '크게 떨어진 뒤 돌아오지 못한 회사는 지금 데이터에 남아 있지 않습니다. ' +
      '분석을 돌리면 이 이야기를 다시 자세히 적어 두겠습니다.</p>';
